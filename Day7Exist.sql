...
SELECT urun_id,musteri_isim
FROM mayis_satislar
WHERE EXISTS ( SELECT urun_id      -- yukarıdaki kodla aynı işlemi yapar
                FROM nisan_satislar
                WHERE nisan_satislar.urun_id=mayis_satislar.urun_id);
/*----------------------------------------------------------------
...
SELECT *             -- BURADA deger girilmemis olan kaitlari getiriyoruz
FROM insanlar
WHERE isim IS NULL;
DELETE FROM insanlar  -- BURADA DA deger girilmemis kayitlari siliyoruz
WHERE isim IS NULL;
UPDATE insanlar      -- BURADA DA deger girilmemis kayitlari UPDATE ediyoruz
SET isim = 'ISIMSIZ'
WHERE isim IS NULL;
/* ======================== ORDER BY ===========================
...
FROM insanciklar
ORDER BY adres;
-- Isim' i Mine olanlari soyisim sirali olarak listeleyen Query yaziniz.
SELECT *
FROM insanciklar
WHERE isim='Mine'
ORDER BY soyisim;
-- Insanciklar tablosundaki soyismi BULUT olanlari isim sirali olarak listeleyin.
SELECT *
FROM insanciklar
WHERE soyisim='BULUT'
ORDER BY isim;
-- Insanciklar tablosundaki tum kayitlari SSN numarasi buyukten kucuge olarak siralayin.
SELECT *
FROM insanciklar
ORDER BY ssn DESC ;
